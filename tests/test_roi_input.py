...
from blender_terrain.core.crs import UTMWorkArea, split_bbox_by_utm_zone
...
class ROIInputTests(unittest.TestCase):
    def test_builds_metric_rectangle_around_projected_center(self) -> None:
        ...
        area = split_bbox_by_utm_zone(bounds)[0]
        projected = project_work_area_bounds(UTMWorkArea(bounds, area.crs))
    ...
    def test_supports_canary_regcan_zone(self) -> None:
        bounds = bbox_from_center_size(-16.6291, 28.2916, 500.0, 500.0)

        self.assertEqual(split_bbox_by_utm_zone(bounds)[0].crs.epsg, 4083)

    def test_rejects_invalid_dimensions_and_unsupported_centres(self) -> None:
        examples = (
            (-3.7, 40.4, 0.0, 100.0),
            (-3.7, 40.4, 100.0, -1.0),
            (20.0, 40.0, 100.0, 100.0),
        )
