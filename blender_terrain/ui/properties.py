...
_IMAGERY_ITEMS_CACHE: list[tuple[str, str, str]] = []
_PRODUCT_ITEMS = (
    ...
    (
        ...
        "DTM (Scottish LiDAR Phase 1, 1 m)",
        "Official historic terrain tiles with sparse Scottish coverage",
        0,
        17,
    ),
    (
        "GB_SCT_SRSP_PHASE1_DSM",
        "DSM (Scottish LiDAR Phase 1, 1 m)",
        "Official historic surface tiles with sparse Scottish coverage",
        0,
        18,
    ),
)
...
def _imagery_product_items(
    properties: object, context: bpy.types.Context
) -> list[tuple[str, str, str]]:
    try:
        available = set(json.loads(properties.available_imagery_product_ids_json))
    except (json.JSONDecodeError, TypeError):
        available = set()
    items = (
...
def _invalidate_validation(properties: object, context: bpy.types.Context) -> None:
    ...
    properties.show_creation_section = False
...
def _data_source_changed(properties: object, context: bpy.types.Context) -> None:
    ...
    properties.available_imagery_product_ids_json = "[]"
...
def _gpkg_layer_items(properties: object, context: bpy.types.Context) -> list[tuple[str, str, str]]:
    try:
...
class BLENDERTERRAIN_ROIProperties(bpy.types.PropertyGroup):
    ...
    available_imagery_product_ids_json: StringProperty(default="[]", options={"HIDDEN"})
    ...
    elevation_resolution: EnumProperty(
        ...
        description=(
            "Output pixel spacing in metres; smaller values preserve more detail but "
            "increase download, processing, memory and mesh cost"
        ),
        items=(
            (
                "AUTO",
                "Auto",
                "Choose the finest resolution that fits the selected resource profile",
            ),
            *tuple(
                (
                    str(value),
    ...
    resource_profile: EnumProperty(
        ...
        description=(
            "Safety budget used to limit raster pixels, decoded textures and terrain "
            "geometry before work starts"
        ),
        items=(
            ("CONSERVATIVE", "Conservative", "Lower memory and geometry limits"),
            ("BALANCED", "Balanced", "Recommended safety limits for most workstations"),
            ("LARGE", "Large (may freeze Blender)", "Higher limits may exhaust RAM or GPU memory"),
        ),
        default="BALANCED",
        update=_invalidate_validation,
    )
    tiling_mode: EnumProperty(
        name="Terrain Object Grid",
        description=(
            "Choose how the ROI is divided into Blender objects; elevation and imagery "
            "remain aligned on the same tile boundaries"
        ),
        items=(
            (
                "AUTOMATIC",
                "Automatic",
                "Choose object dimensions that keep individual meshes manageable",
            ),
            (
                "MANUAL",
                "Manual Grid",
                "Set the number of terrain object rows and columns per projected CRS",
            ),
        ),
        default="AUTOMATIC",
        update=_invalidate_validation,
    ...
    imagery_product: EnumProperty(
        name="Imagery Product",
    ...
    bathymetry_mode: EnumProperty(
        name="Marine Elevation",
        description="Choose how submerged terrain is represented",
        items=(
            (
                "GEBCO",
                "GEBCO Seabed Elevation",
                "Use GEBCO bathymetry for marine cells",
            ),
            (
                "FLAT",
                "Flat Water Level",
                "Do not download bathymetry; submerged cells remain at shoreline level",
            ),
        ),
        default="GEBCO",
        update=_invalidate_validation,
    )
    show_acquisition_section: BoolProperty(default=True, options={"HIDDEN"})
    show_creation_section: BoolProperty(default=False, options={"HIDDEN"})
    show_imported_section: BoolProperty(default=False, options={"HIDDEN"})
    show_cache_section: BoolProperty(default=False, options={"HIDDEN"})
    ...
    job_progress: FloatProperty(default=0.0, min=0.0, max=1.0, subtype="FACTOR", options={"HIDDEN"})
    ...
    adjust_viewport_clip_end: BoolProperty(
        ...
        description=("Set each 3D viewport Clip End to 10^12 metres after terrain creation"),
        default=True,
    )
    ...
    terrain_vertical_scale: FloatProperty(name="Vertical Scale", default=1.0, min=0.001, max=100.0)
    ...
    terrain_smooth_angle: FloatProperty(
        name="Smooth Angle",
        description="Edges sharper than this angle remain sharp",
        default=0.0,
        min=0.0,
        max=3.1415926536,
        subtype="ANGLE",
    ...
    selected_smooth_angle: FloatProperty(
        name="Smooth Angle",
        description="Edges sharper than this angle remain sharp",
        default=0.0,
        min=0.0,
